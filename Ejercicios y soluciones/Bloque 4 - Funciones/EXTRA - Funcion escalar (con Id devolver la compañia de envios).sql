...
CREATE FUNCTION F_NombreCompañiaENVSegunId
(@idpedido INT)
RETURNS NVARCHAR(50)
AS
	BEGIN
		...
		SET @NombreCompañia = (SELECT NombreCompañía 
								FROM  [COMPAÑÍAS DE ENVÍOS] -- ESTA MIERDA ES IMPORTANTE, SI NO, OLVIDATE DE LLAMAR A LA TABLA.
								WHERE IdCompañíaEnvíos = (SELECT FormaEnvío 
															FROM  Pedidos 
